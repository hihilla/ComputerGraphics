...
public class Plain extends Shape {
	...
	public Hit intersect(Ray ray) {
		...
		Point pointOnPlane = pointOnPlane();
		Vec pOnPlaneMinRay = pointOnPlane.sub(ray.source());
		double normalDotRay = normal.dot(ray.direction());
		Vec temp = new Vec(pOnPlaneMinRay.x = pointOnPlane.x / normalDotRay,
				pointOnPlane.y = pointOnPlane.y / normalDotRay,
				pointOnPlane.z = pointOnPlane.z / normalDotRay);
		double t = normal.dot(temp);

		//checking which direction is the right one through the normal and its' opposite
		Vec oppositeDir = new Vec(-ray.direction().x, -ray.direction().y, -ray.direction().z);
		Ray oppsiteRay = new Ray(ray.source(), oppositeDir);
		double cosTheta = (normal.dot(oppositeDir)) / (normal.length() * oppositeDir.length());
		if (cosTheta > 0){
			return new Hit(t, normal);
		} else {
			Vec oppNormal = new Vec(-normal.x, -normal.y, -normal.z);
			return new Hit(t, oppNormal);
		}
